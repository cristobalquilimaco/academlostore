...
function printArticles(store){
    ...
    for (const article of store.articles){
            const articleSoldOut = article.quantity ? `<i class='bx bx-plus' id="${article.id}"></i>` 
            : "<span class='soldOut'>Sold out</span>"

        html += `
        <div class="article">
        <div class="articlePic">
        <img src="${article.image}" alt="sweter" />
        </div>

            <div class="articleInfo">
            <p>${article.name} / <span><b>Stock</b>: ${article.quantity}</p>
            <p>
            $${article.price}.00 
            ${articleSoldOut}
            </p>
...
async function pattern(){
        const store = {
            articles:
        JSON.parse(window.localStorage.getItem("articles")) || (await getMerchadise()),
